test_that("GnRH marker scores merge with automatic score detection", {
  ...
  expect_true(
    is.na(matrix["ECEL1", "B"])
  )

  expect_equal(
    attr(matrix, "score_columns"),
    c(
      A = "avg_log2FC",
      B = "avg_log2FC"
    )
...
test_that("row scaling preserves missing markers and centers singletons", {
  ...
  # A singleton has no estimable row variance and is therefore centered at 0.
  expect_equal(
    unname(
      result$plot_matrix[
        "singleton",
  ...
  # Missing datasets remain missing in the displayed matrix.
  expect_true(
    all(
      is.na(
        result$plot_matrix[
  ...
  # A row observed in at least two datasets can be standardized normally.
  expect_true(
    all(
      is.finite(
        result$plot_matrix[
