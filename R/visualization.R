...
#'
#' Creates a multi-panel quality-control dashboard summarizing
#' GnRH detection performance, classification behavior, threshold
#' optimization, ROC performance, module enrichment, developmental
#' staging, and run parameters from a \code{GnRHcell} analysis.
#'
#' The report automatically extracts diagnostics stored in
#' \code{object@misc$gnrh} after running the detection pipeline.
#'
#' Included panels may include:
#' \itemize{
#'   \item Signal landscape: GNRH1 expression versus composite GnRH score
#'   \item Score distribution by GnRH classification
#'   \item Threshold optimization curve (sensitivity, specificity, F1)
#'   \item ROC curve with AUC estimate
#'   \item Module hit burden versus composite score
#'   \item Classification enrichment across total module hits
#'   \item Developmental stage signal landscape (if staging available)
#'   \item Run parameter summary table
#' }
#'
#' Status and stage legends include both cell counts and percentages
#' for easier interpretation.
#'
#' ROC curves are generated automatically when valid binary labels
#' are available in diagnostic metadata.
#'
#' @param object A \code{Seurat} object processed with
#'   \code{detect_gnrh()} and \code{gnrh_diagnostics()}.
#' @param style Character scalar specifying the plotting theme style
#'   passed to \code{plot_theme()}. Default is \code{"test"}.
#' @param verbose Logical; if \code{TRUE}, progress messages are printed.
#'
#' @return A \code{patchwork} plot object containing the diagnostic dashboard.
#'
#' @details
#' Diagnostic information is expected in:
#' \itemize{
#'   \item \code{object@misc$gnrh$diagnostics}
#'   \item \code{object@misc$gnrh$threshold_curve}
#'   \item optional staging metadata in \code{object$gnrh_stage}
#'   \item optional run parameters in \code{object@misc$gnrh_params}
#' }
#'
#' If staging results are unavailable, the stage panel is replaced
#' by a placeholder panel.
#'
...
gnrh_report <- function(object, style = "test", verbose = TRUE) {
  ...
  if (is.null(g)) {
    stop(
      "Missing object@misc$gnrh. Run detect_gnrh() and gnrh_diagnostics() first.",
      call. = FALSE
    )
  }
  ...
  params <- object@misc$gnrh_params %||%
    g$params %||%
    object@misc$gnrh$run_info$params %||%
    NULL
  ...
  missing <- setdiff(required, colnames(diag))
  ...
  # -------------------------------------------------------#
  # helper: labels with counts and percentages
  # -------------------------------------------------------#

  .legend_labels <- function(x, cols) {

    tab <- table(x)
    levels_use <- intersect(names(cols), names(tab))

    labels <- paste0(
      levels_use,
      " (n = ",
      as.integer(tab[levels_use]),
      ", ",
      round(100 * as.numeric(tab[levels_use]) / sum(tab), 1),
      "%)"
    )

    stats::setNames(labels, levels_use)
  }
  ...
  status_levels <- intersect(names(status_cols), names(table(diag$status)))
  status_labels <- .legend_labels(diag$status, status_cols)

  # -------------------------------------------------------#
  # stage legend labels
  # -------------------------------------------------------#

  has_stage <- "gnrh_stage" %in% colnames(object[[]])

  if (has_stage) {
    md <- object[[]]
    stage_cols <- gnrh_colors("stage")
    stage_levels <- intersect(names(stage_cols), names(table(md$gnrh_stage)))
    stage_labels <- .legend_labels(md$gnrh_stage, stage_cols)
  }
  ...
  p1 <- ggplot2::ggplot(
    ...
    ggplot2::labs(
      title = paste0("Signal landscape by status"),
      x = "GNRH1 expression",
  ...
  # -------------------------------------------------------#
  # Panel 2: stage-colored signal landscape
  # -------------------------------------------------------#
  ...
  if (has_stage) {
    ...
    p2 <- ggplot2::ggplot(
      ...
      ggplot2::labs(
        title = "Signal landscape by stage",
        x = "GNRH1 expression",
        y = "GnRH composite score",
        color = "Stage"
      ) +
      plot_theme(style = style, leg.pos = c(0.28, 0.8))

  } else {

    p2 <- ggplot2::ggplot() +
      ggplot2::theme_void() +
      ggplot2::labs(title = "Stage unavailable")
  }
  ...
  # -------------------------------------------------------#
  # Panel 4: threshold curve
  # -------------------------------------------------------#

  if (
    !is.null(curve) &&
    all(c("threshold", "sensitivity", "specificity", "F1") %in% colnames(curve))
  ) {
    ...
    p4 <- ggplot2::ggplot() +
      ggplot2::theme_void() +
      ggplot2::labs(title = "Threshold curve unavailable")
  }
  ...
  p6 <- ggplot2::ggplot(
    ...
    ggplot2::labs(
      title = "Module signal vs score",
      x = "Total module hits",
      y = "Score",
      color = "Status"
    ) +
    plot_theme(style = style, leg.pos = "none")

  # -------------------------------------------------------#
  # Panel 7: hit enrichment by status
  # -------------------------------------------------------#
  ...
  p7 <- ggplot2::ggplot(
    ...
    ggplot2::labs(
      title = "Enrichment across module hits",
      x = "Total hits",
      y = "Fraction",
      fill = "Status"
    ) +
  ...
  # -------------------------------------------------------#
  # Panel 8: module hit composition
  # -------------------------------------------------------#
  ...
  if (!is.null(params) && length(params) > 0) {
    ...
    param_df <- data.frame(
      ...
      value = vapply(params, .format_param, character(1)),
      stringsAsFactors = FALSE
    )

    param_df$y <- rev(seq_len(nrow(param_df)))
    ...
    p9 <- ggplot2::ggplot(param_df) +
      ...
      ggplot2::xlim(0, 1) +
      ggplot2::labs(title = "Run parameters") +
      ggplot2::theme_void() +
      ggplot2::theme(
    ...
    p9 <- ggplot2::ggplot() +
      ggplot2::theme_void() +
      ggplot2::labs(title = "Run parameters unavailable")
  }
...
#'
#' @param programs Output list returned by \code{\link{gnrh_marker_programs}}.
#' @param table Character. Which result table to visualize. One of
#'   \code{"summary"}, \code{"high_confidence"}, or
#'   \code{"candidate_table"}. Default is \code{"summary"}.
#' @param type Character. Plot type to generate. One of \code{"bar"},
#'   \code{"dot"}, or \code{"tile"}. For gene-level tables
#'   (\code{"high_confidence"} or \code{"candidate_table"}), this argument is
#'   ignored and a gene-level dotplot is returned.
#' @param min_genes Integer. Minimum number of genes required for a
#'   program/dataset combination to be displayed when
#'   \code{table = "summary"}. Default is \code{1}.
#' @param style Character. Theme style passed to \code{\link{plot_theme}}.
#'   One of \code{"classic"}, \code{"minimal"}, \code{"bw"}, \code{"test"},
#'   \code{"void"}, \code{"dirty"}, or \code{"gray"}. Default is
#'   \code{"classic"}.
#' @param mode Character. Theme mode passed to \code{\link{plot_theme}}.
#'   One of \code{"light"} or \code{"dark"}. Default is \code{"light"}.
#' @param txtsize Numeric. Base text size passed to \code{\link{plot_theme}}.
#'   Default is \code{12}.
#' @param x.ang Numeric. Rotation angle for x-axis labels. Default is
#'   \code{45}.
#'
#' @return A \code{ggplot2} object.
#'
#' @details
#' When \code{table = "summary"}, three plot types are available:
#'
#' \itemize{
#'   \item \code{type = "bar"}: barplot of candidate marker counts per
#'   developmental program, faceted by dataset.
#'   \item \code{type = "dot"}: dotplot showing developmental program
#'   enrichment across datasets.
#'   \item \code{type = "tile"}: heatmap-like tile plot showing the number of
#'   candidate genes per developmental program and dataset.
#' }
#'
#' When \code{table = "high_confidence"} or
#' \code{table = "candidate_table"}, the function returns a gene-level
#' dotplot with genes on the y-axis, developmental programs on the x-axis,
#' point color representing confidence level, and point size representing
#' specificity score.
#'
#' The plotting theme is controlled by \code{\link{plot_theme}}, the internal
#' GnRHcell theme system.
#'
#' @examples
#' \dontrun{
...
plot_gnrh_marker_programs <- function(
  ...
  df <- df[df$coexpr_GNRH1 == TRUE, , drop = FALSE]

  if (nrow(df) == 0) {
    stop("No GNRH1 co-expressed genes found in selected table.", call. = FALSE)
  }
